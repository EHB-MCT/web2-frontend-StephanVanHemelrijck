...
L.Control.PrintRouteControl = L.Control.extend({
    ...
    onAdd: function (map) {
        ...
        l.title =
            "Paper-to-World scale and resolution of the printed raster map in Dots Per Inch (DPI). The color of the DPI value indicates the expected print quality, from worst (0, red) to best (300, green). Hover on the labels below to see more help information.";
        l.style.cursor = "help";
        ...
        l.title =
            "Enter a margin to require the route to be contained in a sequence of rectangles that are smaller than the paper. Useful for countering printer bleed, ensuring an overlap to make the route easier to follow across pages, and to ensure a minimum of contextual map area around the route.";
        l.style.cursor = "help";
        p = createElement("p");
        ...
        this.inputPages.title =
            'Comma-separated list of (ranges of) pages to print. For example, "1, 3-5, 7" prints page 1, 3, 4, 5 and 7. Clear to reset to all pages.';
        this.inputPages.addEventListener(
            "change",
            function () {
        ...
        var header = createElement(
            "p",
            { innerHTML: "<b>Print route settings</b>" },
            { margin: "0", fontSize: "13px", padding: divControls.style.borderSpacing }
        ); // padding should be same as borderSpacing in divControls
        var button = createElement(
            "a",
            { innerHTML: "✖", href: "#" },
            { display: "inline-block", width: "30px", height: "30px", lineHeight: "30px", fontSize: "22px" }
        );
        var help = createElement(
            "a",
            {
                innerHTML: "?",
                title: "You get what you see! Zoom the map to your preferred level of detail, modify these settings and hit Print. The color of the DPI value indicates the print quality.",
                href: "#",
            },
            { display: "inline-block", width: "30px", height: "30px", lineHeight: "30px", fontSize: "22px", cursor: "help" }
        );
        ...
        this.inputDPI.addEventListener(
            "change",
            function (event) {
        ...
        this.inputPreset.addEventListener(
            "change",
            function (event) {
                if (this.inputPreset.selectedIndex > 0) {
                    // 0 is "free"
    ...
    printRouteWrapper: async function (print) {
        ...
        var dpi1 = 0,
            hue1 = 0; // horrible print quality  (red)
        var dpi2 = 300,
            hue2 = 140; // excellent print quality (green)
    ...
    getRouteRectangles: function (ll, w, h, p) {
        ...
        for (var i = 0; i < rects.length; i++) {
            ...
            L.rectangle(bigRect, {
                ...
                fillOpacity: this.rectFillOpacity,
            }).addTo(this.rectGroup);
            L.rectangle(smallRect, { stroke: true, weight: 1, opacity: this.rectStrokeOpacity, color: this.rectStrokeColor, fill: false }).addTo(
                this.rectGroup
            );
        }
